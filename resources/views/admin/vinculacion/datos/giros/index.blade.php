<x-app-layout>
  ...
  <x-slot name="slot">
    ...
    <div class="w-full flex justify-end mb-4">
      <a href="{{route('giros.create')}}" class="btn btn-blue">Nuevo</a>
    </div>
    <table id="giros" class="w-full text-sm text-left text-gray-500 dark:text-gray-500">
      ...
      <tbody>
        @forelse ($giros as $giro)
          <tr class="bg-white hover:bg-gray-200 border-b" onclick="window.location='{{route('giros.edit', $giro)}}'" style="cursor: pointer">
            <td class="px-6 py-4">{{$giro->nombre}}</td>
            <td class="px-6 py-4">{{$giro->descripcion}}</td>
          </tr>
        @empty
        @endforelse
      </tbody>
