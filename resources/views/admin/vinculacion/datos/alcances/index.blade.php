<x-app-layout>
  ...
  <x-slot name="slot">
    ...
    <div class="w-full flex justify-end mb-4">
      <a href="{{route('alcances.create')}}" class="btn btn-blue">Nuevo</a>
    </div>
      {{-- <table id="alumnos" class="w-full text-sm text-left text-gray-500 dark:text-gray-400"> --}}
    <table id="alcances" class="w-full text-sm text-left text-gray-500 dark:text-gray-500">
      ...
      <tbody>
        @forelse ($alcances as $alcance)
          <tr class="bg-white hover:bg-gray-200 border-b" onclick="window.location='{{route('alcances.edit', $alcance)}}'" style="cursor: pointer">
            <td class="px-6 py-4">{{$alcance->nombre}}</td>
            <td class="px-6 py-4">{{$alcance->descripcion}}</td>
          </tr>
        @empty
        @endforelse
      </tbody>
