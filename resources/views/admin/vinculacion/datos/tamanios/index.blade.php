<x-app-layout>
  ...
  <x-slot name="slot">
    ...
    <div class="w-full flex justify-end mb-4">
      <a href="{{route('tamanios.create')}}" class="btn btn-blue">Nuevo</a>
    </div>
    <table id="tamanios" class="w-full text-sm text-left text-gray-500 dark:text-gray-500">
      ...
      <tbody>
        @forelse ($tamanios as $tamanio)
          <tr class="bg-white hover:bg-gray-200 border-b" onclick="window.location='{{route('tamanios.edit', $tamanio)}}'" style="cursor: pointer">
            <td class="px-6 py-4">{{$tamanio->nombre}}</td>
            <td class="px-6 py-4">{{$tamanio->descripcion}}</td>
          </tr>
        @empty
        @endforelse
      </tbody>
