<x-app-layout>
  ...
  <x-slot name="slot">
    ...
    <div class="w-full flex justify-end mb-4">
      <a href="{{route('residencias.create')}}" class="btn btn-blue">Nuevo</a>
    </div>
      {{-- <table id="alumnos" class="w-full text-sm text-left text-gray-500 dark:text-gray-400"> --}}
    <table id="residencias" class="w-full text-sm text-left text-gray-500 dark:text-gray-500">
      ...
      <tbody>
        @forelse ($residencias as $residencia)
          <tr class="bg-white hover:bg-gray-200 border-b" onclick="window.location='{{route('residencias.edit', $residencia)}}'" style="cursor: pointer">
            <td class="px-6 py-4">{{$residencia->nombre}}</td>
            <td class="px-6 py-4">{{$residencia->matricula}}</td>
          </tr>
        @empty
        @endforelse
      </tbody>
