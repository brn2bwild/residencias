<x-app-layout>
  ...
  <x-slot name="slot">
    ...
    <div class="w-full flex justify-end mb-4">
      <a href="{{route('estancias.create')}}" class="btn btn-blue">Nueva</a>
    </div>
    <table id="estancias" class="w-full text-sm text-left text-gray-500 dark:text-gray-500">
      ...
      <tbody>
        @forelse ($estancias as $estancia)
          <tr class="bg-white hover:bg-gray-200 border-b" onclick="window.location='{{route('estancias.edit', $estancia)}}'" style="cursor: pointer">
            <td class="px-6 py-4">{{$estancia->nombre}}</td>
            <td class="px-6 py-4">{{$estancia->descripcion}}</td>
          </tr>
        @empty
        @endforelse
      </tbody>
