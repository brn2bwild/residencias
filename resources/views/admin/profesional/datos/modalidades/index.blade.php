<x-app-layout>
  ...
  <x-slot name="slot">
    ...
    <div class="w-full flex justify-end mb-4">
      <a href="{{route('modalidadesr.create')}}" class="btn btn-blue">Nueva</a>
    </div>
    <table id="modalidades" class="w-full text-sm text-left text-gray-500 dark:text-gray-500">
      ...
      <tbody>
        @forelse ($modalidades as $modalidad)
          <tr class="bg-white hover:bg-gray-200 border-b" onclick="window.location='{{route('modalidadesr.edit', $modalidad)}}'" style="cursor: pointer">
            <td class="px-6 py-4">{{$modalidad->nombre}}</td>
            <td class="px-6 py-4">{{$modalidad->descripcion}}</td>
          </tr>
        @empty
        @endforelse
      </tbody>
