<x-app-layout>
  ...
  <x-slot name="slot">
    ...
    <div class="w-full flex justify-end mb-4">
      <a href="{{route('opciones.create')}}" class="btn btn-blue">Nueva</a>
    </div>
    <table id="opciones" class="w-full text-sm text-left text-gray-500 dark:text-gray-500">
      ...
      <tbody>
        @forelse ($opciones as $opcion)
          <tr class="bg-white hover:bg-gray-200 border-b" onclick="window.location='{{route('opciones.edit', $opcion)}}'" style="cursor: pointer">
            <td class="px-6 py-4">{{$opcion->nombre}}</td>
            <td class="px-6 py-4">{{$opcion->descripcion}}</td>
          </tr>
        @empty
        @endforelse
      </tbody>
